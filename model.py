...
class SpanFusionLM(nn.Module):
    ...
    def span_forward_pass(self,
        ...
        if g is not None:
            ...
            g_hat, gate_logits_for_loss = self.gate_net(mean_entropy_for_gate.unsqueeze(-1), train=self.training and not is_teacher_path)
        ...
        for t in range(g_loop):
            ...
            if current_pick_mask.any():
                ...
                if z_picked.numel() > 0:
                    ...
                    seq_K_part = seq[:, n_prompt:]
                    seq_K_part[current_pick_mask] = tok_hat
                    seq[:, n_prompt:] = seq_K_part
            ...
            if t < g_loop - 1 and current_pick_mask.any() and not filled_mask.all():
                ...
                if tokens_for_incremental_pass.shape[1] > 0:
                    ...
                    h_dec_full[:, start_pos_incremental_global : n_prompt + K, :] = h_dec_delta
                    
                    # 【修改】更新 Encoder 使用的 decoder 隐状态，使其始终与最新 h_dec 对齐
                    h_dec_K_positions_detached = h_dec_full[:, n_prompt:, :].detach()
