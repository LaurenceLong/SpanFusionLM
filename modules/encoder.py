...
class SpanEncoderSelfAttention(nn.Module):
    ...
    def forward(self,
        ...
        cos, sin = self.rotary_emb(
            value_states,
            seq_len=position_ids.max().item() + 1 if position_ids.numel() > 0 else k_len,
...
class SpanEncoderCrossAttention(nn.Module):
    ...
    def forward(self,
        ...
        cos_q, sin_q = self.rotary_emb_q(
            query_states,
            seq_len=position_ids_q.max().item() + 1 if position_ids_q.numel() > 0 else k_len_q,
...
class SpanEncoder(nn.Module):
    ...
    def step(self,
        ...
        k_indices_relative_to_span_start = torch.arange(K, device=device)
        global_position_ids_k = prompt_len + k_indices_relative_to_span_start.unsqueeze(0).expand(bsz, K)

        # Attention masks for encoder:
        self_attn_mask = torch.zeros(bsz, 1, K, K, dtype=torch.bool, device=device)
        cross_attn_mask = torch.zeros(bsz, 1, K, K, dtype=torch.bool, device=device)

        # <-- 关键修改：使用 clone() 以免原始 latent_z 被就地修改影响反向传播
        output_z = latent_z.clone()
